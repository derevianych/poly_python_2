...
file_path = r"D:\Richard\Files\Personal files\Polytech\Additional\4th semester\Python\recipeitems\recipeitems.json"
recipes = pd.read_json(file_path, lines=True)
...
# - периоды или временныеинтервалы - есть конечная и начальная точка. "март 2025 года", "24 часа"
...
d = datetime.datetime(year = 2026, month = 3, day = 4)
print(d)
print(type(d))
...
# print(t+t1) нельзя сложить, разные единицы измерения (дни и месяцы, нет четкой совместимости)
...
# секунды можно сложить с миллисекундами, а дни с неделями
...
d2 = d + pd.to_timedelta(np.arange(30), "D") # 20:15
print(d2)
print(type(d2))
...
# Работа с временными рядами
# - конкретные моменты времени, т. е абсолютные значения "1 января 2026 года 00:00
# Pands: Timestamp (numpy.datetime64)
# - периоды или временныеинтервалы - есть конечная и начальная точка. "март 2025 года", "24 часа"
# Pands: Period (numpy.datetime64) - PeriodIndex
# - продолжительность (временная дельта) = 15 минут
# Pands: Timedelta (nupmpy.timedelta64) - Timedeltaindex
...
# Коды периодичности
# M Q A - конец
# MS QS AS - начало месяца, квартала, года
